...
* organize the data
** split by director, writer, and cast
* clean up the data
* model the data
** initial naive model
** bootstrapped model
...
Some episodes have more than one director, so I separate them into separate rows.
```{r}
df_directors <- df %>% 
...
The original data contains mispellings of director names, which will cause issues when I filter out less common directors. This code fixes those mispellings.
```{r}
...
This selects only directors that were involved in more than 2 episodes.
```{r}
df_directors <- df_directors %>%  
