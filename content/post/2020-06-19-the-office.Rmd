...
* organize the data
** split by role (director, writer, and cast)
* clean up the data
* model the data
* review model output and performance

The first step is to download the data from the ```{shrute}``` package. The data is organized by season/episode/line, which is useful for analyzing characters, but not writers or directors.
```{r}
...
This selects only the characters that were involved in more than 20 episodes and pivots the data wide.
```{r}
...
The first model I create will be a linear model with ridge regression. This penalizes extreme model coefficients.
```{r eval=FALSE}
...
make sure model is using normalized data
```{r}
wf %>%
  add_model(tune_spec) %>% 
  fit(office_train) %>% 
...
```lowest_rmse``` finds the ```penalty``` value that gives the lowest RMSE. This graph shows that increasing the penalty increases model performance up to a point.
```{r}
...
Splitting the data by season shows that the model predicted ```imdb_rating``` reasonably well for most seasons. It overstimated season 1 and underestimated season 3.
```{r}
final_ridge %>% 
  fit(office_train) %>% 
  predict(office_train) %>% 
...
df_vi %>% 
  ...
  labs(y = NULL)
```
Phyllis is the cast MVP, based on this model. The high values for "Everyone" and "All" show that scenes where the entire office is involved are highly associated with increased ratings. Carell The model has a dim view of Nellie and the later seasons of the show in general. The negative importance for Roy clearly means that the model is internally valid and cannot be improved upon.
